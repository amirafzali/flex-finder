...
if __name__ == '__main__':
    ...
    db = firebase.db  # this connects to our Firestore database
    ...
    if command and type(command) == str:
        ...
        if command == 'r':
            ...
            workout_types = re.sub(r'\s+', '',
                                   input("Enter valid workout type: string seperated commas (eg; 'cardio, core, ...'"))
            ...
            profile.create_a_user(username, gender, gyms, school, workout_types, timeslots_available,uid)

        elif command == 'l':
            firebase.sign_in(email, password)
            uid = firebase.uid
            profile.existing_user(uid)
        ...
        while running:
            ...
            if len(input_lst) == 1:
                if command == "exit":
                    ...
                    timeslots_available = eval(input("Enter timeslots as a 2D list [[day,start_time,end_time]]: eg; [['mon',1,2]]").strip())

                    print(search(db, username=username, timeslots=timeslots_available))
                    ...
                    print(search(db, school=school, workout_type=workout_type, gym=gym, timeslots=timeslots_available))
                    ...
                    gym = input("enter gym: (eg; DBAC, Gym1, etc)").strip()
                    timeslots_available = eval(input("Enter timeslot as a 2D list [day,start_time,end_time]: eg; ['mon',1,2]").strip())
                ...
                elif command == "delete_appointment":
                    username = input("Enter username").strip()
                    appointment_id = input("Enter appointment id: eg: -MnqfPhsUyTssIEMcwGg").strip()

                    delete_appointment(db, username=recipient_username, appointment_id=appointment_id)
